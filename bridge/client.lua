...
PoliceDispatch = function(data)
    if Config.Dispatch == 'linden_outlawalert' then
        local d = {displayCode = '911', description = 'Telah Terjadi Perampokan Warung', priority = 'high', isImportant = 1, recipientList = Config.JobPolisi, length = '20000', infoM = 'fa-info-circle', info = 'Check Map Untuk Lokasinya'}
        local dispatchData = {dispatchData = d, caller = 'Alarm', coords = data.coords}
        TriggerServerEvent('wf-alerts:svNotify', dispatchData)
    elseif Config.Dispatch == 'cd_dispatch' then
        local data = exports['cd_dispatch']:GetPlayerInfo()
        TriggerServerEvent('cd_dispatch:AddNotification', {
            job_table = Config.JobPolisi,
            coords = data.coords,
            title = 'Perampokan Warung',
            message = 'Telah terjadi perampokan warung di ' ..data.street,
            flash = 0,
    ...
    elseif Config.Dispatch == 'qs-dispatch' then
        ...
        TriggerServerEvent('qs-dispatch:server:CreateDiapatchCall', {
            job = Config.JobPolisi,
            callLocation = playerData.coords,
            callCode = { code = '10-88', snippet = 'Perampokan Warung' },
            message = 'Telah terjadi perampokan warung di ' ..playerData.street_1,
            flashes = false,
            image = nil,
            blip = {
                sprite = 488,
